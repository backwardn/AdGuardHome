...
func createObject(filename string, limit int) *statsCtx {
	s := statsCtx{}
	s.limit = limit * 24
	s.filename = filename
...
func convertTopArray(a []countPair) []map[string]interface{} {
...
func (s *statsCtx) GetData(timeUnit TimeUnit) map[string]interface{} {
	...
	d["avg_processing_time"] = float64(sum.TimeAvg/uint(len(units))) / 1000000
